...
class Mininet_wifi(Mininet):
	...
	if 'sidedB' in params:
        ...
        self.addSectorParamToNode(node, wlans, params)
        self.addMaxSectorParamToNode(node, wlans, params)
        self.addMaindBParamToNode(node, wlans, params)
        self.addSidedBParamToNode(node, wlans, params)
...
# -----------------------------------------------------------
    ...
    @staticmethod
    def addSectorParamToNode(node, wlans=0, params=None,
                              isVirtualIface=False):
        "Add Sector Param"
        if isVirtualIface:
            node.params['sector'].append(node.params['sector'][0])
        else:
            node.params['sector'] = []
            if 'sector' in params:
                sector_list = params['sector'].split(',')
                for sector in sector_list:
                    node.params['sector'].append(int(sector))
            else:
                for _ in range(wlans):
                    node.params['txpower'].append(0) # default 0

    @staticmethod
    def addMaxSectorParamToNode(node, wlans=0, params=None,
                              isVirtualIface=False):
        "Add MaxSector Param"
        if isVirtualIface:
            node.params['max_sector'].append(node.params['max_sector'][0])
        else:
            node.params['max_sector'] = []
            if 'max_sector' in params:
                max_sector_list = params['max_sector'].split(',')
                for max_sector in max_sector_list:
                    node.params['max_sector'].append(int(max_sector))
            else:
                for _ in range(wlans):
                    node.params['max_sector'].append(4) # default 4

    @staticmethod
    def addMaindBParamToNode(node, wlans=0, params=None,
                              isVirtualIface=False):
        "Add maindB Param"
        if isVirtualIface:
            node.params['maindB'].append(node.params['maindB'][0])
        else:
            node.params['maindB'] = []
            if 'maindB' in params:
                maindB_list = params['maindB'].split(',')
                for maindB in maindB_list:
                    node.params['maindB'].append(int(maindB))
            else:
                for _ in range(wlans):
                    node.params['maindB'].append(20) # default 20

    @staticmethod
    def addSidedBParamToNode(node, wlans=0, params=None,
                              isVirtualIface=False):
        "Add SidedB Param"
        if isVirtualIface:
            node.params['SidedB'].append(node.params['SidedB'][0])
        else:
            node.params['SidedB'] = []
            if 'SidedB' in params:
                SidedB_list = params['SidedB'].split(',')
                for SidedB in SidedB_list:
                    node.params['SidedB'].append(int(SidedB))
            else:
                for _ in range(wlans):
                    node.params['SidedB'].append(5) # default 5
                ...
                for node in nodes:
                    for wlan in range(0, len(node.params['wlan'])):
                        ...
                        node.setAntennaGain(node.params['antennaGain'][wlan],
                                            ...
                                            setParam=False)
                        # --------------------
                        node.setSector(node.params['sector'][wlan],
                                            intf=node.params['wlan'][wlan],
                                            setParam=False)
                        node.setMaxSector(node.params['max_sector'][wlan],
                                            intf=node.params['wlan'][wlan],
                                            setParam=False)
                        node.setMaindB(node.params['maindB'][wlan],
                                            intf=node.params['wlan'][wlan],
                                            setParam=False)
                        node.setSidedB(node.params['sidedB'][wlan],
                                            intf=node.params['wlan'][wlan],
                                            setParam=False)
                        # -----------------------
