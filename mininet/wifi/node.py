...
# ------------------------------------------
    ...
    def setSector(self, value, intf=None, setParam=True):
        "Set Sector"
        wlan = self.params['wlan'].index(intf)
        #self.pexec('iw dev %s set txpower fixed %s'
        #           % (intf, (int(value) * 100)))
        self.params['sector'][wlan] = value
        self.setSectorWmediumd(wlan)

    def setMaxSector(self, value, intf=None, setParam=True):
        "Set MaxSector"
        wlan = self.params['wlan'].index(intf)
        #self.pexec('iw dev %s set txpower fixed %s'
        #           % (intf, (int(value) * 100)))
        self.params['txpower'][wlan] = value
        self.setMaxSectorWmediumd(wlan)

    def setMaindB(self, value, intf=None, setParam=True):
        "Set MaindB"
        wlan = self.params['wlan'].index(intf)
        #self.pexec('iw dev %s set txpower fixed %s'
        #           % (intf, (int(value) * 100)))
        self.params['maindB'][wlan] = value
        self.setMaindBWmediumd(wlan)

    def setSidedB(self, value, intf=None, setParam=True):
        "Set SidedB"
        wlan = self.params['wlan'].index(intf)
        #self.pexec('iw dev %s set txpower fixed %s'
        #           % (intf, (int(value) * 100)))
        self.params['sidedB'][wlan] = value
        self.setSidedBWmediumd(wlan)
# --------------------------------------
...
# ------------------------------------------
    def setSectorWmediumd(self, wlan):
        "Set Sector for wmediumd"
        if wmediumd_mode.mode == WmediumdCst.INTERFERENCE_MODE:
            sector_ = self.params['sector'][wlan]
            WmediumdServer.update_sector(WmediumdSector(
                self.wmIface[wlan], int(sector_)))

    def setMaxSectorWmediumd(self, wlan):
        "Set MaxSector for wmediumd"
        if wmediumd_mode.mode == WmediumdCst.INTERFERENCE_MODE:
            maxsector_ = self.params['max_sector'][wlan]
            WmediumdServer.update_maxsector(WmediumdMaxSector(
                self.wmIface[wlan], int(maxsector_)))

    def setMaindBWmediumd(self, wlan):
        "Set MaindB for wmediumd"
        if wmediumd_mode.mode == WmediumdCst.INTERFERENCE_MODE:
            maindB_ = self.params['maindB'][wlan]
            WmediumdServer.update_maindB(WmediumdMaindB(
                self.wmIface[wlan], int(maindB_)))

    def setsidedBWmediumd(self, wlan):
        "Set Antenna Gain for wmediumd"
        if wmediumd_mode.mode == WmediumdCst.INTERFERENCE_MODE:
            sidedB_ = self.params['sidedB'][wlan]
            WmediumdServer.update_sidedB(WmediumdSidedB(
                self.wmIface[wlan], int(sidedB_)))
# ----------------------------------------
